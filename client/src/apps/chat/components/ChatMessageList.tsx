...
export function ChatMessageList(props: {
  ...
  const handleConversationRestartFrom = React.useCallback(
  ...
  const handleConversationTruncate = React.useCallback(
    (messageId: string) => {
      const messages = getConversation(conversationId)?.messages;
  ...
  const handleTextDiagram = React.useCallback(
  ...
  const handleTextImagine = React.useCallback(
    async (text: string) => {
      if (!isImaginable) return openLayoutPreferences(2);
      if (conversationId) {
        setIsImagining(true);
  ...
  const handleTextSpeak = React.useCallback(
    async (text: string) => {
      if (!isSpeakable) return openLayoutPreferences(3);
      setIsSpeaking(true);
  ...
  const handleSelectAll = (selected: boolean) => {
    ...
    if (selected) for (const message of conversationMessages) newSelected.add(message.id);
  ...
  const handleSelectionDelete = () => {
    if (conversationId) for (const selectedMessage of selectedMessages) deleteMessage(conversationId, selectedMessage);
